...
public class ThEUpgradeInventory extends ThEInternalInventory {
    ...
    public boolean isItemValidForSlot(int index, ItemStack stack) {
        if (stack.getItem() instanceof ItemKnowledgeCore && !isKnowledgeCoreSlot()) return false;
        if (!(stack.getItem() instanceof IUpgradeModule)
                && !ThEApi.instance().upgrades().getUpgrade(stack).isPresent()) return false;
        if (this.upgradable
                == null) // If the item/block/part that this is attached to is null, then just allow
            // without checking max allowed
            return true;
        return this.getMaxUpgrades(stack) > 0
                && this.getUpgrades(stack) < this.getMaxUpgrades(stack);
    }
    ...
    public int getUpgrades(Object o) {
        if (!this.cached) this.calculateUpgrades();
        ...
        if (o instanceof ItemStack) {
            ...
            if (stack.getItem() instanceof IUpgradeModule) { // AE Upgrade
                ...
                if (upgrade.isPresent()) o = upgrade.get();
            }
        }
                ...
                .forEachRemaining(
                        stack -> {
                            ...
                            ThEApi.instance()
                                    ...
                                    .ifPresent(
                                            upgrade ->
                                                    this.cachedUpgrades.put(
                                                            upgrade,
                                                            this.cachedUpgrades.getOrDefault(
                                                                            upgrade, 0)
                                                                    + stack.getCount()));
                            ...
                            if (stack.getItem() instanceof IUpgradeModule) {
                                ...
                                this.cachedUpgrades.put(
                                        upgrade,
                                        this.cachedUpgrades.getOrDefault(upgrade, 0)
                                                + stack.getCount());
                            }
                        });
        this.cached = true;
    ...
    private int getMaxUpgrades(ItemStack upgradeStack) {
        ...
        ThEApi.instance()
                .upgrades()
                .getUpgrade(upgradeStack)
                .ifPresent(upgrade -> max.set(upgrade.getSupported(this.upgradable)));
        if (upgradeStack.getItem() instanceof IUpgradeModule) {
            ...
            if (upgrade != null) {
                ...
                Optional<ItemStack> upgradable =
                        stream.filter(o -> ForgeUtil.areItemStacksEqual(this.upgradable, o))
                                .findFirst();
                upgradable.ifPresent(
                        stack -> max.set(upgrade.getSupported().getOrDefault(upgradable.get(), 0)));
            }
        }
        return max.get();
    }
