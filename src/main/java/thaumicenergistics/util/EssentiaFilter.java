...
public class EssentiaFilter implements INBTSerializable<NBTTagCompound>, Iterable<Aspect> {
    ...
    public Aspect getAspect(int slot) {
        if (slot < 0 || slot > this.aspects.length) return null;
    ...
    public boolean isInFilter(String aspect) {
        for (Aspect af : this.aspects)
            if (af != null && af.getTag().equalsIgnoreCase(aspect)) return true;
        return false;
    }

    public boolean hasAspects() {
        for (Aspect a : this.aspects) if (a != null) return true;
        return false;
    }
    ...
    public void deserializeNBT(NBTTagCompound tag) {
        if (tag.hasKey("count")) this.aspects = new Aspect[tag.getInteger("count")];
        for (int i = 0; i < this.aspects.length; i++)
