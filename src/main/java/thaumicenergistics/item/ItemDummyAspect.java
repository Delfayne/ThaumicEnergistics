...
/**
 * A dummy item that uses the aspect icon for its icon Used by AEEssentiaStack
 *
 * <p>TODO: Render item based on Aspect Image + Colour
 *
 * @author BrockWS
 */
public class ItemDummyAspect extends ItemBase implements IThEModel {
        ...
        return stack.hasTagCompound()
                ? Aspect.getAspect(
                        stack.getTagCompound() != null
                                ? stack.getTagCompound().getString("aspect")
                                : null)
                : null;
    }
    ...
    public void initModel() {
        this.setTileEntityItemStackRenderer(DummyAspectRenderer.INSTANCE);
        // ThEModelLoader.MODEL_LOADER.addModel("models/item/dummy_aspect", new
        // DummyAspectItemModel());
        ModelLoader.setCustomModelResourceLocation(
                this,
                0,
                new ModelResourceLocation(Reference.MOD_ID + ":dummy_aspect", "inventory"));
        // ModelLoader.setCustomModelResourceLocation(this, 0, new
        // ModelResourceLocation(this.getRegistryName(), "inventory"));
    }
}
