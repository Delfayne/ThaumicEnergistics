...
public class ThETheOneProbe implements IThEIntegration, Function<ITheOneProbe, Void> {
    ...
    public void preInit() {
        FMLInterModComms.sendFunctionMessage(
                this.getModID(), "getTheOneProbe", this.getClass().getName());
    }

    @Override
    public Void apply(ITheOneProbe registrar) {
        registrar.registerProvider(new TileTOPDataProvider());
        registrar.registerBlockDisplayOverride(
                new PartTOPDisplayOverride()); // Fixes our parts showing up as AE2 parts (lets AE2
        // add the power-state info, due to inheritance)
        return null;
    }
}
