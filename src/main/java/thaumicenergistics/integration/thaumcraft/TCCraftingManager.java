...
public class TCCraftingManager {
    ...
    public static IArcaneRecipe findArcaneRecipe(IItemHandler handler, EntityPlayer player) {
        ...
        return recipe instanceof IArcaneRecipe
                        && ThaumcraftCapabilities.knowsResearch(
                                player, ((IArcaneRecipe) recipe).getResearch())
    ...
    public static float getDiscount(EntityPlayer player) {
        ...
        for (int i = 0; i < 4; i++) {
            ...
            if (stack.isEmpty() || !(stack.getItem() instanceof IVisDiscountGear)) continue;
        ...
        if (player.isPotionActive(PotionVisExhaust.instance)) {
            level1 =
                    Objects.requireNonNull(player.getActivePotionEffect(PotionVisExhaust.instance))
                            .getAmplifier();
        }
            ...
            level2 =
                    Objects.requireNonNull(
                                    player.getActivePotionEffect(
                                            PotionInfectiousVisExhaust.instance))
                            .getAmplifier();
        }
        if (level1 > 0 || level2 > 0) discount -= (Math.max(level1, level2) + 1) * 10;
        return discount / 100f;
    }

    private static class ArcaneInventoryCrafting extends InventoryCrafting
