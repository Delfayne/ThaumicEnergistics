...
public class AEAspectRegister {
    ...
    private class AEItemInfo {
        ...
        private void addPredefinedBonuses() {
            ...
            // Grinder recipe?
            if (this.recipe instanceof IGrinderRecipe) {
            ...
            // Sky stone
            else if (this.definition.equals(aeBlocks.skyStoneBlock())) {
                // Add tene
                this.bonusAspects.add(Aspect.DARKNESS, 3);
            }

            // Sky chest
            else if (this.definition.equals(aeBlocks.skyStoneChest())) {
                // Add vac
            ...
            // Cell workbench
                ...
                // Add cloth
                this.bonusAspects.add(Aspect.CRAFT, 2);
            }
            // ME Interface
            ...
            else if (this.definition.equals(aeBlocks.craftingAccelerator())) {
        ...
        private boolean buildAspectsFromGrinder() {
            try {
                for (IGrinderRecipe recipe : AEAspectRegister.this.GRINDER_RECIPES) {
                    ...
                    if (ThEUtil.areItemStacksEqual(recipeOutput, this.itemStack)) {
                ...
                if ((stackAspects == null) || (stackAspects.size() == 0)) {
                    ...
                    if (ingInfo != null) {
                        ...
                        if (!AEAspectRegister.this.DEPENDENCY_CHAIN.contains(ingInfo)) {
                            ...
                            stackAspects = AspectHelper.getObjectAspects(stack);
                        }
                    }
                }
        ...
        private void getIngredientsForRecipe() {
            ...
            if (this.recipe instanceof appeng.recipes.game.FacadeRecipe) {
                // Get ingredients
                ingredientObjects = new NonNullList[]{((FacadeRecipe) this.recipe).getIngredients()};
            } else if (this.recipe instanceof appeng.recipes.game.DisassembleRecipe) {
                // Get ingredients
                ingredientObjects = ((appeng.recipes.game.DisassembleRecipe) this.recipe).getIngredients().toArray();
            } else if (this.recipe instanceof IGrinderRecipe) {
                // Get ingredient
                ingredientObjects = new Object[] { ((IGrinderRecipe) this.recipe).getInput() };
                ...
                // Get ingredients
                ingredientObjects = new NonNullList[]{((ShapedRecipes) this.recipe).recipeItems};
        ...
        public boolean equals(final Object obj) {
            if (obj instanceof AEItemInfo) {
                ...
                return this.isMatch(((IItemDefinition) obj).maybeStack(1).orElse(ItemStack.EMPTY));
            }
        ...
        public AspectList getFinalAspects() {
            ...
            if (aspectCount > 6) {
                Aspect[] aspects = finalAspects.getAspects();
        ...
        public boolean isMatch(final ItemStack stack) {
            return ThEUtil.areItemStacksEqual(stack, this.itemStack);
        }
        ...
        public void registerItem(final int pass) {
            ...
            if (this.itemStack != null) {
                ...
                if (!this.itemStack.getItem().getClass().getCanonicalName().startsWith("appeng")) {
                    // ThELog.info( "Skipping registration for non AE item: %s (%s) ", this.itemStack.getDisplayName(),
                    // this.itemStack.getItem().getClass().getCanonicalName() );
                    return;
                }
    ...
    Collection<IGrinderRecipe> GRINDER_RECIPES = null;
    ...
    private void registerBase() {
        ...
        // Skystone
        Item skyStone = aeBlocks.skyStoneBlock().maybeItem().orElse(ItemStack.EMPTY.getItem());
        if (skyStone != ItemStack.EMPTY.getItem()) {
            aspects = new AspectList();
            aspects.add(Aspect.ELDRITCH, 2);
            aspects.add(Aspect.EARTH, 1);
            aspects.add(Aspect.DARKNESS, 1);
            this.registerItem(aeBlocks.skyStoneBlock(), aspects);
        ...
        this.registerCableSet(aeParts.cableDenseCovered(), aspects);
        ...
        p2pAspects.add(Aspect.DESIRE, 4);
        ...
        this.registerItem(aeParts.p2PTunnelFluids(), aspects);
        // Light
        aspects = p2pAspects.copy();
        ...
        this.registerItem(aeParts.p2PTunnelFE(), aspects);
    ...
    private void registerThEItems() {
        ...
        // Get an AE 1K Cell
        ItemStack aeCell = AEApi.instance().definitions().items().cell1k().maybeStack(1).orElse(ItemStack.EMPTY);
        AspectList cellAspects = null;
        if (aeCell != null) {
            // Get the aspect list for a 1k cell
            AspectList aeCellAspects = AspectHelper.getObjectAspects(aeCell);
            if (aeCellAspects != null) {
                ...
                if (aspectCount > 4) {
                    Aspect[] OrderedAspects = cellAspects.getAspects();
                ...
                ThaumcraftApi.registerObjectTag(teItems.essentiaCell1k().maybeStack(1).get(), cellAspects);
                ThaumcraftApi.registerObjectTag(teItems.essentiaCell4k().maybeStack(1).get(), cellAspects);
                ThaumcraftApi.registerObjectTag(teItems.essentiaCell16k().maybeStack(1).get(), cellAspects);
                ThaumcraftApi.registerObjectTag(teItems.essentiaCell64k().maybeStack(1).get(), cellAspects);
                //ThaumcraftApi.registerObjectTag(teItems.EssentiaCell_256k.getStack(), cellAspects);
        ...
        ThaumcraftApi.registerObjectTag(new ItemStack(teBlocks.ironGearbox().maybeBlock().get()), ironGearboxAspects);
        ...
        ThaumcraftApi.registerObjectTag(new ItemStack(teBlocks.thaumiumGearbox().maybeBlock().get()), thaumGearbox);
    }
    ...
    public void registerAEAspects() {
        ...
        if (!this.getGrinderRecipes()) {
            ThELog.warn("Unable to load AE2 Grinder recipes, aspect registration will be incomplete");
        }

        // Get the inscriber recipes
        if (!this.getInscriberRecipes()) {
            ThELog.warn("Unable to load AE2 Inscriber recipes, aspect registration will be incomplete");
        }
