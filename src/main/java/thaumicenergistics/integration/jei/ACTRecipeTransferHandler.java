 ...
 * @author Alex811
 */
public class ACTRecipeTransferHandler<C extends ContainerArcaneTerminal>
    ...
    public IRecipeTransferError transferRecipe(
        ...
        ingredients.forEach(
                (i, o) -> {
                    ...
                    if (displayed != null
                            && !displayed.isEmpty()) // Try using the displayed one first
                    subTypes.appendTag(o.getDisplayedIngredient().serializeNBT());
                    o.getAllIngredients()
                            .forEach(
                                    stack -> {
                                        if (!ForgeUtil.areItemStacksEqual(
                                                stack,
                                                displayed)) // No point sending the same stack twice
                                        subTypes.appendTag(stack.serializeNBT());
                                    });
                    if (o.isInput()) normalIngredients.appendTag(subTypes);
                    else crystalIngredients.appendTag(subTypes);
                });
        tag.setTag("normal", normalIngredients);
        tag.setTag("crystal", crystalIngredients);
        // TODO: Actually check and report errors
        if (doTransfer) PacketHandler.sendToServer(new PacketJEIRecipe(tag));
        return null;
    }
}
