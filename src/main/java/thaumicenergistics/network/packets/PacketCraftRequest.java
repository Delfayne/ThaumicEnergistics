    ...
    public static class Handler implements IMessageHandler<PacketCraftRequest, IMessage> {
        ...
        public IMessage onMessage(PacketCraftRequest message, MessageContext ctx) {
            ...
            thread.addScheduledTask(
                    () -> {
                        if (!(player.openContainer instanceof ContainerCraftAmountBridge)) return;
                        ContainerCraftAmountBridge ca =
                                (ContainerCraftAmountBridge) player.openContainer;
                        if (!(ca.getTarget() instanceof PartBase)
                                || !(ca.getTarget() instanceof IThEGridHost)
                                || ca.getItemToCraft() == null) return;
                        ...
                        try {
                            ...
                            GuiHandler.openGUI(
                                    ...
                                    player,
                                    part.getLocation().getPos(),
                                    part.side);
                            if (player.openContainer instanceof ContainerCraftConfirmBridge) {
                                ContainerCraftConfirmBridge cc =
                                        (ContainerCraftConfirmBridge) player.openContainer;
