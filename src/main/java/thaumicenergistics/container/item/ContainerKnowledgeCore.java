...
public class ContainerKnowledgeCore extends ContainerBase {
        ...
        for (int i = 0; i < SLOT_NUM; i++) {
            ...
            if (KnowledgeCoreUtil.hasRecipe(knowledgeCoreStack, i)) {
                KnowledgeCoreUtil.Recipe recipe =
                        KnowledgeCoreUtil.getRecipe(knowledgeCoreStack, i);
        ...
        if (ForgeUtil.isServer()) {
                ...
                switch (packet.action) {
                    case KNOWLEDGE_CORE_ADD:
                        ...
                        ThEInternalInventory ingredients =
                                (ThEInternalInventory)
                                        ((InvWrapper) inscriber.getInventory("crafting")).getInv();
                        ItemStack result = inscriber.getInventory("result").getStackInSlot(0);
                        KnowledgeCoreUtil.setRecipe(
                                knowledgeCoreStack,
                                packet.index,
                                new KnowledgeCoreUtil.Recipe(
                                        ingredients, result, inscriber.getCurrentRequiredVis()));
                        break;
            ...
            if (KnowledgeCoreUtil.isEmpty(knowledgeCoreStack))
                ThEApi.instance()
                        ...
                        .ifPresent(
                                blank ->
                                        ((InvWrapper) inscriber.getInventory("upgrades"))
                                                .getInv()
                                                .setInventorySlotContents(0, blank));
            GuiHandler.openGUI(
                    ModGUIs.ARCANE_INSCRIBER,
                    player,
                    inscriber.getPartPos(),
                    inscriber.getPartSide());
        }
    }
    ...
    public void playWriteSound(EntityPlayer player) {
        player.world.playSound(
                player,
                inscriber.getPartPos(),
                new SoundEvent(ThEApi.instance().sounds().knowledgeCoreWrite()),
