...
public abstract class ContainerSharedEssentiaBus extends ContainerBaseConfigurable
    ...
    protected void bindUpgradesInventory(
            IItemHandler handler, int offsetX, int offsetY, int count) {
        for (int i = 0; i < count; i++) {
            this.addSlotToContainer(
                    new SlotUpgrade(handler, i, offsetX, offsetY + 18 * i) {
                        @Override
    ...
    public void recalculateSlots() {
        for (Slot slot : this.inventorySlots)
            if (slot instanceof SlotGhost)
                ((SlotGhost) slot)
                        .setSlotEnabled(
                                ((SlotGhost) slot).getSlotGroup() <= this.getCapacityUpgrades());
    }

    @Override
    public EssentiaFilter getEssentiaFilter() {
        if (this.part == null) return null;
        ...
        if (item instanceof ItemMaterial || item instanceof appeng.items.materials.ItemMaterial)
            ...
            if (!config.isInFilter(aspect)) {
                this.inventorySlots.stream()
                        ...
                        .findFirst()
                        .ifPresent(
                                essentiaSlot -> config.setAspect(aspect, essentiaSlot.slotNumber));
            }
        }
    }
