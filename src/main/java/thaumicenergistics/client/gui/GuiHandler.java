...
public class GuiHandler implements IGuiHandler {
    ...
    public static void openGUI(
            ModGUIs gui, EntityPlayer player, BlockPos pos, AEPartLocation side) {
        if (gui == null) throw new IllegalArgumentException("gui cannot be null!");
        else if (player == null) throw new IllegalArgumentException("player cannot be null!");

        if (pos != null)
            player.openGui(
                    ThaumicEnergistics.INSTANCE,
                    GuiHandler.calculateOrdinal(gui, side),
    ...
    public static int calculateOrdinal(ModGUIs gui, AEPartLocation side) {
        if (side == null) side = AEPartLocation.UP;
    ...
    public Object getServerGuiElement(
            int ordinal, EntityPlayer player, World world, int x, int y, int z) {
        TileEntity te = null;
        ...
        switch (guiID) {
            ...
            case AE2_CRAFT_STATUS:
                return new ContainerCraftingStatusBridge(
                        player.inventory, (PartSharedTerminal) part);
            case AE2_PRIORITY:
    ...
    public Object getClientGuiElement(
        ...
        switch (guiID) {
            case ARCANE_ASSEMBLER:
                return new GuiArcaneAssembler(
                        new ContainerArcaneAssembler(player, (TileArcaneAssembler) te));
            case ESSENTIA_IMPORT_BUS:
                return new GuiEssentiaImportBus(
                        new ContainerEssentiaImportBus(player, (PartEssentiaImportBus) part));
            case ESSENTIA_EXPORT_BUS:
                return new GuiEssentiaExportBus(
                        new ContainerEssentiaExportBus(player, (PartEssentiaExportBus) part));
            case ESSENTIA_STORAGE_BUS:
            ...
            case ARCANE_INSCRIBER:
                return new GuiArcaneInscriber(
                        new ContainerArcaneInscriber(player, (PartArcaneInscriber) part));
            case AE2_CRAFT_AMOUNT:
            ...
            case KNOWLEDGE_CORE_DEL:
            case KNOWLEDGE_CORE_VIEW:
                return new GuiKnowledgeCore(
                        new ContainerKnowledgeCore(player, guiID, player.openContainer));
            default:
                return null;
        }
    }
