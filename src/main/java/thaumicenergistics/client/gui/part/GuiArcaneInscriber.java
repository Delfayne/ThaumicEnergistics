...
public class GuiArcaneInscriber extends GuiArcaneTerminal {
    ...
    public void initGui() {
        ...
        coreButtons.add(
                coreAddButton =
                        new GuiImageButton(this.getGuiLeft() + 87, coreBtnRowY, 0, 12, images));
        coreButtons.add(
                coreDelButton =
                        new GuiImageButton(this.getGuiLeft() + 104, coreBtnRowY, 0, 7, images));
        coreButtons.add(
                coreViewButton =
                        new GuiImageButton(this.getGuiLeft() + 121, coreBtnRowY, 1, 12, images));
    ...
    protected void actionPerformed(GuiButton button) {
        ...
        if (button == coreAddButton
                && !knowledgeCore.isEmpty()
                && hasRecipe
                && ((ContainerArcaneInscriber) this.container).recipeIsArcane
                && !recipeExists) {
    ...
    protected void drawGuiContainerForegroundLayer(int mouseX, int mouseY) {
        ...
        this.fontRenderer.drawString(
                ThEApi.instance()
                        .lang()
                        .guiVisRequired()
                        .getLocalizedKey(this.visRequired > -1 ? this.visRequired : 0),
                60,
                this.getYSize() - 168,
                4210752);
        this.fontRenderer.drawString(
                ThEApi.instance().lang().guiArcaneInscriber().getLocalizedKey(), 8, 6, 4210752);
        this.fontRenderer.drawString(
        ...
        if (!knowledgeCore.isEmpty()) {
            ...
            if (currentIsBlank) {
                ...
                if ((coreViewButton.isHovered() || coreDelButton.isHovered()))
                    renderText(
                            ThEApi.instance().lang().guiKnowledgeCoreBlank().getLocalizedKey(),
                            mouseX,
                            mouseY);
            } else {
            ...
            if (coreAddButton.isHovered()) {
                if (hasRecipe) {
                    if (!hasArcaneRecipe)
                        renderText(
                                ThEApi.instance().lang().guiRecipeNotArcane().getLocalizedKey(),
                                mouseX,
                                mouseY);
                    else if (recipeExists)
                        renderText(
                                ThEApi.instance().lang().guiRecipeAlreadyStored().getLocalizedKey(),
                                mouseX,
                                mouseY);
                } else
                    renderText(
                            ThEApi.instance().lang().guiNoRecipe().getLocalizedKey(),
                            mouseX,
                            mouseY);
            }
        } else {
            ...
            renderButton(coreViewButton, false);
            renderButton(coreDelButton, false);
            if ((coreViewButton.isHovered()
                    || coreDelButton.isHovered()
                    || coreAddButton.isHovered()))
                renderText(
                        ThEApi.instance().lang().guiInsertKnowledgeCore().getLocalizedKey(),
                        mouseX,
                        mouseY);
        }
    }
    ...
    protected void renderButton(GuiImageButton button, boolean enabled) {
        ...
        if (enabled) {
            ...
            if (button == coreDelButton) coreDelButton.setImageAlpha(1.0F, 1.0F);
                ...
                button.setAllImages(
                        images, (button == coreViewButton) ? button.width : 0, button.height * 11);
        } else {
            coreDelButton.setImageAlpha(0.5F, 0.5F);
            if (button == coreDelButton) coreDelButton.setImageAlpha(0.5F, 0.5F);
            else
                button.setAllImages(
                        images, (button == coreViewButton) ? button.width : 0, button.height * 12);
        }
    }

    protected void renderText(String text, int x, int y) {
        this.drawCenteredString(
                mc.fontRenderer, text, x - this.getGuiLeft(), y - this.getGuiTop() - 5, 0x00e6ac);
    }
}
