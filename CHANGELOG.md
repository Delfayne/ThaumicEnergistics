...
* Efficiency changes around Essentia Storage Bus - protections against making a full AE2 rebuild on
  every change event. As a result, these buses now tick (like AE2 ones) and look for deltas in the
  attached container's current state vs what they last saw it being, and update AE2 with this delta
* All parts now only check their facing, not every single neighbour face
* Essentia Terminals now correctly recalculate their opacity when light sources around them change
* Preventing a double-render call on placing an Essentia Storage Bus
* Prevent jars from voiding when you sneak right-click a part onto them
...
* Fixed a duplication bug where ghost items in the Arcane Inscriber would drop when broken or
  wrenched
...
* Creative Essentia Cells now list MAX_INT essentia instead of 1000. They were previously still
  infinite, but with
  a cap of 1000 present in the AE system would prohibit large crafts.

# 2022/07/09 - 2.2.5 Release

* Corrected Essentia cell partitioning. Previously cells could be partitioned, but this was being
  ignored. Now
  partitioned cells will only accept the filtered essentia.
* Improved net handling code around Arcane Crafting Terminal to handle diffs of the AE2 inventory
  changes rather than
  the entire inventory. Should be a bit faster.
* Fixing Essentia export buses not respecting export cards. This continued to work on the first
  aspect in the list,
  without checking if it was needed or not. Now this will pass to the next essentia if none would be
  transferred.

## Build

* Fixed a number of issues around the use of the CurseMaven plugin, probably fallout from the
  CurseForge API
  changes lately. Builds simply broke with missing dependencies, and some version numbers for
  dependent mods seemed
  shifted.
